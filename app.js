...
//for direct line
var secret = 'oMz68RtGlVU.cwA.DpQ.PxQJ1rndXUqelWjnID-yqrZfbjY8xO6fYkrfET6dF4E';
var _tokenObject;
    ...
    (tokenObject) => {
      ...
      client.initConversationStream(_tokenObject).subscribe(
        (message) => {
          ...
          rule1.second = times;
          _refershSchedule1 = schedule.scheduleJob(rule1, function () {
            if (!_handling) {
              _handling = true;
              // console.log('1111111111111111111===>' + new Date());
              getmessagefrombotframework1(_tokenObject, _watermark);
            }
          });
...
function getmessagefrombotframework1(tokenobject, sendwatermark) {
  var senduserid = 'oBEU-wqrGTXM9OxsI6h5QTvHAYF4';
  client.getMessage(tokenobject, sendwatermark).subscribe(
    (result) => {
      ...
      _.forEach(result.activities, (activity)=>{
          if(!_.isEmpty(activity.replyToId)) {
            //send message to wechat client
            sendMessageToClient(senduserid, [activity]);
            //if send message max , then restart the converstation
            if (activity.replyToId) {
              var arr = activity.replyToId.split('|');
              // console.log(arr[1]);
              if (arr[1] == '9999999') {
                getTokenAndGetConverstation();
              }
...
app.use('/bot/wechat', wechat(config, wechat.text(function (message, req, res, next) {
  //------------------------------------------------------------------------
